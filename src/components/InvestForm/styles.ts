...
export const LabelOptions = styled.label<LabelOptionsType>`
  ...
  font-size: 1em;
  font-weight: 500;
  color: ${props => (props.isActive ? '#ffff' : 'var(--text-label)')};
  cursor: pointer;
  ...
  background: ${props => (props.isActive ? 'var(--orange)' : 'transparent')};
...
export const Label = styled.label<LabelType>`
  color: ${props => (props.isWrong ? '#ff0000' : '')};
`;
...
export const ButtonSubmit = styled.button<ButtonSubmitType>`
  background: ${props => (props.isValidate ? 'var(--orange)' : 'var(--grey)')};
`;
